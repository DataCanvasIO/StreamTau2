...
public interface ServerDtoMapper {
    ServerDtoMapper MAPPER = Mappers.getMapper(ServerDtoMapper.class);

    @Mappings({
        @Mapping(source = "projectAssetId", target = "id"),
        @Mapping(source = "assetName", target = "name"),
        @Mapping(source = "assetType", target = "type"),
        @Mapping(source = "assetDescription", target = "description"),
    })
    ServerDto toDto(Asset asset);
}
