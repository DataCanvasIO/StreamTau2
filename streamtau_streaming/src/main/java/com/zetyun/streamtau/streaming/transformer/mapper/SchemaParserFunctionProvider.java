...
public class SchemaParserFunctionProvider implements Function<Operator, MapFunction<RtEvent, RtEvent>> {
    @Override
    public MapFunction<RtEvent, RtEvent> apply(Operator operator) {
        RtSchemaParser parser = RtSchemaParser.createJsonEventParser(
            ((SchemaParser) operator).getSchema().createRtSchema()
        );
